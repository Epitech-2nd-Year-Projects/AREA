...
SET "metadata" = jsonb_set(
        jsonb_set(
            ...
            to_jsonb('{"query":"query Issues($teamId: ID!) { issues(filter: { team: { id: { eq: $teamId } } }, first: 50) { nodes { id createdAt identifier title description url } } }","variables":{"teamId":"{{params.teamId}}"}}'::text)
        ),
        '{ingestion,http,cursor}',
        '{"key":"linear_issue_cursor","source":"fingerprint"}'::jsonb
    ),
    "updated_at" = NOW()
