...
func TestExecutorSupports(t *testing.T) {
	...
	component := &componentdomain.Component{Name: gmailComponentName, Provider: componentdomain.Provider{Name: gmailProviderName}}
	if !exec.Supports(component) {
		t.Fatalf("expected support for gmail component")
	}
...
func TestExecutorExecuteSuccess(t *testing.T) {
	...
	raw, ok := payload["raw"]
	if !ok {
		t.Fatalf("expected raw payload field")
	}
	bodyBytes, err := base64.URLEncoding.WithPadding(base64.NoPadding).DecodeString(raw)
	if err != nil {
		t.Fatalf("failed to decode body: %v", err)
	}
	if !strings.Contains(string(bodyBytes), "Hello world") {
		t.Fatalf("expected body to contain payload, got %s", string(bodyBytes))
	}
}
...
func TestExecutorExecuteRefreshOnUnauthorized(t *testing.T) {
	...
	client := &stubHTTPClient{
		responses: []*http.Response{
			...
			{StatusCode: http.StatusOK, Body: ioNopCloser("ok")},
		},
	}
...
func (s *stubProvider) Name() string { return gmailProviderName }
...
func (s *stubHTTPClient) Do(req *http.Request) (*http.Response, error) {
	...
	if len(s.responses) == 0 {
		return &http.Response{StatusCode: http.StatusOK, Body: ioNopCloser("ok")}, nil
	}
	resp := s.responses[0]
