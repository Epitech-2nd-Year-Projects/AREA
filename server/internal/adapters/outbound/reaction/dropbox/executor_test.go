...
func TestFolderExecutorSupports(t *testing.T) {
	exec := NewFolderExecutor(nil, nil, nil, nil, nil)

	component := &componentdomain.Component{
		Name: createFolderComponentName,
...
func TestFolderExecutorExecuteCreatesFolder(t *testing.T) {
	userID := uuid.New()
	identityID := uuid.New()
	...
	client := &httpClientStub{
		response: http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{"metadata":{"name":"reports"}}`)),
		},
	}

	exec := NewFolderExecutor(repo, providerResolverStub{}, client, clockStub{now: future.Add(-time.Minute)}, zap.NewNop())

	component := &componentdomain.Component{
		ID:       componentID,
		Name:     createFolderComponentName,
		Provider: componentdomain.Provider{Name: dropboxProviderName},
	}
	...
	if client.lastRequest.URL.String() != dropboxCreateFolderEndpoint {
	...
	if got := payload["path"]; got != "/reports" {
		t.Fatalf("expected path /reports got %v", got)
	}
	if autorename, ok := payload["autorename"].(bool); !ok || autorename {
		t.Fatalf("expected autorename false got %v", payload["autorename"])
	}
}
