...
class ApiConfig {
    ...
    static void initialize() {
      ...
      callbackBaseUrl = dotenv.get('CALLBACK_BASE_URL', fallback: 'http://localhost:8080');
      enableLogging = dotenv.get('ENABLE_LOGGING', fallback: 'true').toLowerCase() == 'true';
      
      final connectTimeoutSeconds = int.tryParse(
        dotenv.get('CONNECT_TIMEOUT_SECONDS', fallback: '15')
      ) ?? 15;
      final receiveTimeoutSeconds = int.tryParse(
        dotenv.get('RECEIVE_TIMEOUT_SECONDS', fallback: '20')
      ) ?? 20;
