...
const onSubmit = async () => {
  ...
  loading.value = true
  try {
    const { error } = await useFetch('/register', {
    ...
    if (error.value) {
      ...
      errorMsg.value = status === 409 ? 'Email already in use' : 'Registration error'
      return
    }
    okMsg.value = 'Account created and signed in!'
...
<template>
  <main class="mx-auto max-w-md px-6 py-10 md:px-8">
    ...
    <form @submit.prevent="onSubmit" class="mt-4 grid gap-3">
      ...
      <label class="grid gap-1">
        <div class="text-sm font-medium text-gray-700">Password (min 8 characters)</div>
        <input
          v-model="password"
          type="password"
          required
